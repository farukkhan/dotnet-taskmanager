...
- **One branch per Jira issue.** Never work directly on `master`. Branch name: `feature/<JIRA-KEY>-<short-description>`.
...
- Commit messages start with the Jira key, e.g. `<JIRA-KEY>: Add complete/reopen endpoints`.

## Testing

There are no test projects yet. The README names **xUnit** as the intended framework.

- Before creating the first test project, propose the layout and get approval. A suggested layout is `tests/Domain.Tests`, `tests/Application.Tests`, and `tests/TaskManager.Api.IntegrationTests`.
...
## Known gaps (as of 2026-09-23; update as they are fixed)

- `CompleteTaskItemUseCase` and `ReopenTaskItemUseCase` exist but are not registered in DI and have no API endpoints.
- There is no global exception handling. `NotFoundException` and `ConcurrencyException` are not mapped to HTTP status codes.
- There are no automated tests yet, although the README mentions them.
- The Blazor front end is experimental and still contains template pages (`Counter`, `Weather`).
